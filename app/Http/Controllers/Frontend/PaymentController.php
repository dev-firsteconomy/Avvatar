...
use Appnings\Payment\Facades\Payment as CCPayment;
...
class PaymentController extends Controller
{
     ...
     */
     ...
     public function response(Request $request)
     {
        ...
        {
            // For default Gateway
            $response = CCPayment::response($request);
            $orderNumberString = str_replace('_',' ',$response['order_id']);
            $orderNumberString = str_replace('O','/',$orderNumberString);
            //dd($response,$response['order_status'],$orderNumberString);

            $order = Order::where('order_number',$orderNumberString)->first();
            //dd($response['order_status'],);
            ...
            if(isset($response['order_status']) && $response['order_status'] === 'Success')
            {
                ...
                try
                {
                    ...
                    if($user->loyalty_points >= $order->loyalty_points )
                    {
                        User::where('id',$user->id)->update([
                                'loyalty_points' => $user->loyalty_points - $order->loyalty_points
                            ]);
                    }    
                    ...
                    UserReward::create([
                            'user_id' => $user->id,
                            'transaction_type' => 0,
                            'amount' => $order->loyalty_points,
                            'remark' => 'Reward points used for Order Number: ' . $order->order_number
                        ]);

                    $userdata = User::where('id',$user->id)->first();
                    $userdata->loyalty_points = $userdata->loyalty_points + $order->loyalty_points_earned;
                    $userdata->save();
                    ...
                    UserReward::create([
                            ...
                            'remark' => 'Reward earned for Order Number: ' . $order->order_number
                        ]);
